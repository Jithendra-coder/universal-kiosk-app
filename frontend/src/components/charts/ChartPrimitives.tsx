...
export function BarChart({ values, color = CHART_COLORS.sales, ariaLabel = "Bar chart" }: { values: ReadonlyArray<Point>; color?: string; ariaLabel?: string }) {
  const id = useId(); const data = points(values); if (!data.length || data.every((point) => point.value === 0)) return <EmptyChart />;
  const max = Math.max(1, ...data.map((point) => point.value)); const width = 640; const height = 220; const padX = 24; const padY = 22; const slot = (width - padX * 2) / data.length; const barWidth = Math.min(36, Math.max(18, slot * .56)); const summary = `Highest ${data.reduce((best, point) => point.value > best.value ? point : best, data[0]).label}: ${Math.max(...data.map((point) => point.value))}.`;
  return <div className="mt-chart" role="img" aria-label={ariaLabel} aria-describedby={id}><svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none"><title>{summary}</title>{[0, .5, 1].map((ratio) => <line key={ratio} x1={padX} x2={width - padX} y1={height - padY - ratio * (height - padY * 2)} y2={height - padY - ratio * (height - padY * 2)} stroke={CHART_COLORS.grid} />)}{data.map((point, index) => { const barHeight = (point.value / max) * (height - padY * 2); return <g key={`${point.label}-${index}`}><rect x={padX + index * slot + (slot - barWidth) / 2} y={height - padY - barHeight} width={barWidth} height={barHeight} rx="5" fill={color}><title>{`${point.label}: ${point.value}`}</title></rect><text x={padX + index * slot + slot / 2} y={height - 2} textAnchor="middle">{point.label}</text></g>; })}</svg><p id={id} className="mt-chart-summary">{summary}</p></div>;
}
