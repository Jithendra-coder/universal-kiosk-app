...
export function PageSection({ className, ...props }: SectionProps) {
  return <section className={cn("mt-page-section", className)} {...props} />;
}
...
export function ResponsiveGrid({ className, columns = 3, ...props }: DivProps & { columns?: 1 | 2 | 3 | 4 }) {
  return <div className={cn("mt-responsive-grid", `mt-responsive-grid--${columns}`, className)} {...props} />;
}
...
export function Inline({ className, gap = 12, style, ...props }: DivProps & { gap?: 4 | 8 | 12 | 16 | 20 | 24 | 32; }) {
  return <div className={cn("mt-inline", className)} style={{ "--mt-inline-gap": `${gap}px`, ...style } as CSSProperties} {...props} />;
}
...
export function ModeSwitch<T extends string>({ value, options, onChange, label = "View mode" }: { value: T; options: ReadonlyArray<{ value: T; label: string }>; onChange: (value: T) => void; label?: string }) {
  return <div className="mt-segmented-control mt-mode-switch" role="group" aria-label={label}>{options.map((option) => <button type="button" key={option.value} aria-pressed={value === option.value} onClick={() => onChange(option.value)}>{option.label}</button>)}</div>;
}
...
export function StickyActionBar({ className, ...props }: DivProps) {
  return <div className={cn("mt-sticky-action-bar", className)} {...props} />;
}

export function UnsavedChangesBar({ message = "You have unsaved changes.", actions, className, ...props }: DivProps & { message?: ReactNode; actions?: ReactNode }) {
  return <div className={cn("mt-unsaved-bar", className)} role="status" {...props}><span>{message}</span>{actions && <div className="mt-unsaved-bar__actions">{actions}</div>}</div>;
}
